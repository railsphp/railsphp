...
class Inflections implements InflectionsInterface
{
    protected $acronyms     = [];
    ...
    public function addAcronym($word)
    {
        $this->acronyms[strtolower($word)] = $word;
        $this->acronymRegex = '/' . implode('|', $this->acronymRegex) . '/';
    }
    
    public function addPlural($rule, $replacement)
    {
        unset($this->uncountables[$rule], $this->uncountables[$replacement]);
        $this->plurals[$rule] = $replacement;
    }
    
    public function addSingular($rule, $replacement)
    {
        unset($this->uncountables[$rule], $this->uncountables[$replacement]);
        $this->singulars[$rule] = $replacement;
    }
    
    public function addIrregular($singular, $plural)
    {
        unset($this->uncountables[$singular], $this->uncountables[$plural]);
    ...
    public function addUncountable()
    {
        $this->uncountables = array_merge($this->uncountables, func_get_args());
    }
    
    public function addHuman($rule, $replacement)
    {
